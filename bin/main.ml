let run_with (type a) (dom : a Rule_enum.Domain.t) forced num_rand
  ...
  let stats_oc = if stats_file <> "" then Some (open_out stats_file) else None in
  let output_oc = if output_file <> "" then Some (open_out output_file) else None in

  (match stats_oc with Some oc ->
  ...
  let rs, _iterations =
    Rule_enum.Algorithm.run ~max_size dom
      ...
      ~on_iteration:(fun (s : Rule_enum.Algorithm.iter_summary) ->
        ...
        Printf.printf "Size %d  [%.1fs]  enum=%d  +SR=%d  +KR=%d  +IR=%d  total: SR=%d KR=%d IR=%d\n%!"
          s.size elapsed s.enumerated nsr nkr nir
        ...
        (match output_oc with Some oc ->
          Printf.fprintf oc "--- Size %d (enumerated %d, %.1fs) ---\n" s.size s.enumerated elapsed;
