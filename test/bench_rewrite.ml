...
let bench_size n =
  ...
  let max_vars = 3 in
  let inputs = Eval.generate_inputs dom 100 max_vars in
  ...
  let rs, _ = Algorithm.run ~max_size:prev dom ~num_random_inputs:0
    ~forced_inputs:inputs ~max_vars in
  ...
  let _ = List.map (fun t -> Rewrite.normalize ~index:norm_index t) enumerated in
  let t_norm = Unix.gettimeofday () -. t0 in
  let t1 = Unix.gettimeofday () in
  let _ = List.map (fun t ->
    let simplified, _ = Rewrite.normalize ~index:norm_index t in
